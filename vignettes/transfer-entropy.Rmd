...
The following scatterplot provides a first illustration of some of the dependencies among both time series.
The left-hand-side shows the relations for lagged $X$, tThe plot in the centre displays the level-level relationship between the two variables (no lags are used) and the right-hand-side shows the relations for lagged $Y$.
...
As in the above example, a scatterplot provides a first illustration of some of the dependencies among both time series.
...
The following graph reports the results for increasing values of quantiles.
```{r te_2a, eval=T, echo=FALSE}
df <- data.frame(
  q1 = 5:25,
  q2 = 95:75
)
...
As shown above, the question how to determine the quantiles is important as it impacts the order of magnitude of the transfer entropy.
...
Due to the proposed symbolic encoding of the data, tail events are associated with a lower likelihood compared to events in the centre of the distribution.
...
Again, a scatterplot provides a first illustration of some of the dependencies among both time series.
...
# combine the lists for each ticker into one data.table
