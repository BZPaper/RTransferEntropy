...
For $0<q<1$  events which have a low probability to occur receive more weight, while for $q>1$ the weights induce a preference for outcomes $j$ with a higher initial probability.
...
Testing for and quantifying the information flow between two time series with Shannon and Rényi transfer entropy, as outlined above, can be implemented with the package `RTransferEntropy`.
...
Some auxilliary functions are written in `C++` using [`Rcpp`](https://github.com/RcppCore/Rcpp) to speed up computations, furthermore we use the [`future`](https://github.com/HenrikBengtsson/future) package to allow parallel computing, allowing for more efficient programming when the `transfer_entropy()` function is called repeatedly or when large amounts of bootstraps are used.
...
We estimate Shannon transfer entropy with the defaults for all function arguments and  using the parallel processing option provided by the [`future`](https://cran.r-project.org/web/packages/future/index.html) package.
...
Consider the above example again, where the results show a significant information flow from $X$ to $Y$, but not vice versa.
...
The following graph reports the results for increasing $\tau$ which specifies the quantiles used as (4+$\tau/96-\tau) such that for $\tau=1$ the results correspond to the previous implementation. 

```{r te_2a, eval=T, include=FALSE}
ete <- NULL
for (i in 1:21){
 ete <- append(ete, calc_ete(x, y, quantiles = c(4+i,96-i)))
}
```
```{r te_2b, eval=T}
plot(ete, axes=FALSE, ylab="ETE(X->Y)", xlab=expression(tau))
axis(2)
axis(1, at = 1:length(ete), labels = 5:25)
...
Statistical significance, is, however, not affected. 
...
A different approach is provided by Rényi transfer entropy which allows to reweight the probabilities associated with the individual bins.
Due to the proposed symbolic encoding of the data, tail events are associated with a lower likelihood as events in the centre of the distribution.
Rényi transfer entropy, thus, puts more weight on the tails in calculating transfer entropy.
This is particularly convenient when the distribution is assumed to be more informative in the tails.  Note again that Rényi transfer entropy estimates can potentially turn out to be negative.
In such a case, it simply means that there is no information transfer from one time series to another.
...
In a first step, we estimate Rényi transfer entropy with the default settings of the `transfer_entropy()` function, i.e. with a weighting parameter $q=0.1$ which gives a heavy weight to the (infrequent) tail observations.
...
We illustrate both effects using different values of $q$.
Note that this result is illustrated using the raw transfer entropy estimates and not the effective transfer entropy estimates because the latter depend on the shuffling procedure. 
...
The Shannon transfer entropy from $X$ to $Y$ is estimated as `r format(round(shannon_te3$coef[1,1],4), nsmall=4)`.
...
Furthermore, it is indeed approaching `r format(round(shannon_te3$coef[1,1],4), nsmall=4)` as $q\rightarrow 1$.
...
However, the information flow from the market to the stocks is higher for most stocks than the other direction.
Also, as can easily be seen from the confidence bounds, the information flow towards the market is not statistically significant in 3 cases (on a 5\% significance level). 

Again, the estimated transfer entropy depends on the choice of the quantile.
This is illustrated in the following Figure which reports density plots of effective transfer entropies for two different choices of quantiles, namely `quantiles = c(5, 95)` in the upper graph and `quantiles = c(10, 90)` in the lower graph.
The solid (dotted) line depicts the information flow from the market to the individual stocks (from the stocks to the market).
