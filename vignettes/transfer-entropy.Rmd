...
author: "Simon Behrendt, Thomas Dimpfl, Fanzsika J. Peter, David J. Zimmermann"
date: "`r Sys.Date()`"
...
$$
where $T_{J\rightarrow I}$ consequently measures the information flow from $J$ to $I$ ( $T_{I \rightarrow J}$ as a measure for the information flow from $I$ to $J$ can be derived analogously). Transfer entropy can also be based on R�nyi entropy rather than Shannon entropy, which depends on a weighting parameter $q$ and can be calculated as

$$
  H^q_J = \frac{1}{1-q} log \left(\sum_j p^q(j)\right), 
$$
with  $q >0$. For $q\rightarrow 1$ R�nyi entropy converges to Shannon entropy. For $0<q<1$  tail events receive more weight, while for $q>1$ the weights on center observations dominate. Consequently, R�nyi entropy provides a more flexible tool for estimating uncertainty, since different areas of a distribution can be emphasized, depending on the parameter $q$. Using the escort distribution $\phi_q(j)=\frac{p^q(j)}{\sum_j p^q(j))}$ with $q >0$ to normalize the weighted distributions, the R?nyi transfer entropy measure is given by
...
$$
Analogously to (Shannon) transfer entropy R�nyi entropy  measures the information flow from $J$ to $I$.
...
Testing for and quantifying the information flow between two time series with Shannon and R�nyi transfer entropy, as outlined above, can now easily be implemented with the package `RTransferEntropy`. The package is installed and loaded in the usual way.
...
for (i in 2:(n+200)){
  ...
  y <- append(y, abs(x[i-1]) + rnorm(1, 0, 1))
}
...
As shown above, the question how to determine the quantiles is important as it impacts on the order of magnitude of the transfer entropy. A different approach is provided by R�nyi transfer entropy which allows to put more weight on the tails in calculating transfer entropy. This is particularly convenient when the distribution is assumed to be more informative in the tails. To illustrate this effect, we simulate data for which the dependence of $Y$ on $X$ changes with the level the innovation.

\begin{eqnarray*}
x_t & = & 0.2x_{t-1} + \varepsilon_{x,t}\\
y_t & = & \begin{cases} x_{t-1} + \varepsilon_{y,t} \quad \text{if } |\varepsilon_{y,t}| > s \\ 0.3x_{t-1} + \varepsilon_{y,t} \quad \text{if } |\varepsilon_{y,t}| < s
\end{cases},
\end{eqnarray*}
...
Giving more weights to the tails should reveal that the information flow is largely occuring in the tails. Therefore, we set q=0.3 and invoque the estimatio of R�nyi transfer entropy as follows:
...
The result indicates that the tails are indeed more informative. The estimated R�nyi effective transfer entropy is slightly higher than the Shannon ETE. Again, the information flow from $Y$ to $X$ is correctly identified as being zero.

Since R�nyi transfer entropy approaches Shannon transfer entropy as $q \rightarrow 1$, we illustrate this property using different values of $q$. Note that this result is illustrated using the normal transfer entropy estimate and not the effective transfer entropy estimates because the latter depend on the shuffling procedure. 
...
As presented above, the Shannon transfer entropy from $X$ to $Y$ is estimated as `r format(round(shannon_te3$coef[1,1],4), nsmall=4)`. Using different values of $q$ we obtain the following results for the R�nyi transfer entropy.
