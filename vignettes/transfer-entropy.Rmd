...
for (i in 2:(n + 200)) {
  ...
  if (eps_x > 2) {
    ...
    y <- append(y, 0.3 * x[i - 1] + rnorm(1, 0, 1))
  }
}
...
Giving more weights to the tails should reveal that the information flow is largely occuring in the tails. Therefore, we set q=0.3 and invoque the estimatio of Rényi transfer entropy as follows:
...
As a direct comparison, we use the data-generating-process from above to test the two different libraries
...
(res1 <- TransferEntropy::computeTE(x, y, embedding = 2, k = 2))

(res2 <- RTransferEntropy::transfer_entropy(x, y, quiet = T))
```

We see that the values are relatively close with `r round(res1$TE, 5)` from the `TransferEntropy::computeTE()`-function and `r round(res2$coef[1, 1], 5)` from our package.

Taking the example from the `computeTE` help-file
```{r}
x <- rep(0, 10000 + 1)
y <- rep(0, 10000 + 1)
...
(res3 <- TransferEntropy::computeTE(x, y, 3, 1, "MI_diff"))

(res4 <- RTransferEntropy::transfer_entropy(x, y, quiet = T))
```

We see a larger difference XXX EXPLANATION WHY!!
