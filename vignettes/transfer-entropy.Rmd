...
Before we turn to different applications below, we already provide a simple example here, in order to demonstrate how the output of `transfer_entropy()` looks like. Let us consider a linear relationship between two random variables $X$ and $Y$, where $Y$ depends on $X$ with one lag and $X$ is orthogonal to $Y$, such that $x_t = 0.2x_{t-1} + \varepsilon_{x,t}$ and $y_t = x_{t-1} + \varepsilon_{y,t}$, with $\varepsilon_{x,t}$ and $\varepsilon_{y,t}$ being standard normally distributed with a mean of 1 and a variance of 2. In this case, $X$ serves as a predictor for $Y$, but not vice versa. These processes are readily implemented taking, for example, 10,000 observations.
...
#> -----------------------------------------------------------------
#>       X->Y      0.0902      0.0892      0.0003      0.0000    ***
#>       Y->X      0.0006      0.0000      0.0003      0.7833       
#> -----------------------------------------------------------------
#> Bootstrapped TE Quantiles (300 replications):
#> -----------------------------------------------------------------
#> Direction        0%       25%       50%       75%      100%
#> -----------------------------------------------------------------
#>     X->Y    0.0003    0.0006    0.0008    0.0011    0.0021
#>     Y->X    0.0002    0.0007    0.0009    0.0012    0.0023
#> -----------------------------------------------------------------
