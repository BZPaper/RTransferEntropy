...
df <- stocks[, .(ete_xy = calc_ete(ret, sp500),
                 ete_yx = calc_ete(sp500, ret)), by = ticker]

df_long <- melt(df, id.vars = "ticker")

# order the ticker by the ete_xy
df_long[, ticker := factor(ticker, levels = unique(ticker[order(df$ete_xy)]))]

# rename the variable (xy/yx)
df_long[, variable := factor(variable, levels = c("ete_xy", "ete_yx"),
                             labels = c("Flow towards Market",
                                        "Flow towards Stock"))]

ggplot(df_long, aes(x = ticker, y = value)) + 
  geom_point() +
  facet_wrap(~variable) +
  theme(axis.text.x = element_text(angle = 90)) +
...
The graphic reports effective transfer entropy estimates for 10 stocks.
The results illustrate that there is indeed a bi-directional information flow.
